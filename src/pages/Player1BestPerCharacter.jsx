...
export default function Player1BestPerCharacter() {
  const { records } = useRecords();

  const bestPerformances = useMemo(() => {
    const p1Performances = getPlayer1Performances(records);
    return getBestPerformancePerCharacter(p1Performances);
  }, [records]);
  ...
  if (bestPerformances.length === 0) {
    return <EmptyState message="No Player 1 character performances available yet." />;
  }

  return (
    <Panel title="Player 1 Best Record Per Character">
